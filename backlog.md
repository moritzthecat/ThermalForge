...
- **Battery-optimized thresholds.** The 88/70 defaults are tuned for AC. On battery, a lower reduce threshold (less power draw → battery preserved and passive cooling aided) and/or different hysteresis may be better. *Why it waits:* needs field data from battery sessions. (0.3.2 makes the reduced assertion actually take effect on battery — pre-0.3.2 the set was written to the AC domain only and was a silent no-op there; the tuning itself still isn't validated.)
- **Weak-AC detection.** A low-wattage adapter (e.g. 30 W on a 90 W machine) can't sustain high performance, so the battery gets stressed/drained when the guard restores it. 0.3.2 tracks the *source* (AC vs battery) via IOPowerSources notifications, but not the *wattage* — IOPowerSources exposes adapter current/voltage for some models, which would derive it. Until then the workaround is guard off + reduced in System Settings (documented in the README).
